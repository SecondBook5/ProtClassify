#!/usr/bin/env python3
# File: model_utils.py
# Description: Utility functions for saving, loading, and tracking machine learning models.

from pathlib import Path
from typing import Union

import joblib
import pandas as pd

from protclassify import paths

PathLike = Union[str, Path]


def _resolve(path: PathLike) -> Path:
    """Resolve a path relative to the project root."""
    path_obj = Path(path)
    return paths.resolve_path(path_obj)


def load_model(model_path: PathLike) -> object:
    """
    Load a saved model from disk.

    Args:
        model_path: Relative or absolute path to the saved model `.joblib` file.

    Returns:
        object: Loaded model object.

    Raises:
        FileNotFoundError: If the model file does not exist.
    """
    model_path = _resolve(model_path)
    if not model_path.exists():
        raise FileNotFoundError(f"Model file not found: {model_path}")

    model = joblib.load(model_path)
    print(f"Model successfully loaded from {model_path}")
    return model


def load_best_model_from_tracker(
    tracker_file: PathLike = paths.MODEL_TRACKER,
    metric: str = "accuracy",
    maximize: bool = True,
) -> object:
    """
    Load the best-performing model from the model tracker based on a selected metric.

    Args:
        tracker_file: Path to the model tracker CSV file.
        metric: Metric used for model selection (default is "accuracy").
        maximize: Whether to maximize (True) or minimize (False) the metric.

    Returns:
        object: Best model loaded from disk.

    Raises:
        Exception: If the tracker file does not exist or contains no models.
    """
    tracker_path = _resolve(tracker_file)
    if not tracker_path.exists():
        raise FileNotFoundError(f"Tracker file not found: {tracker_path}")

    tracker_df = pd.read_csv(tracker_path)

    if tracker_df.empty:
        raise Exception("Tracker file is empty. No models available for loading.")
    if metric not in tracker_df.columns:
        raise ValueError(f"Metric '{metric}' not found in tracker columns.")

    best_idx = tracker_df[metric].idxmax() if maximize else tracker_df[metric].idxmin()
    best_model_row = tracker_df.loc[best_idx]

    model_path = tracker_path.parent / best_model_row["model_file"]
    print(f"Loading best model: {best_model_row['model_file']} with {metric}={best_model_row[metric]}")
    return load_model(model_path)


def save_model(
    model: object,
    model_type: str,
    featureset: str,
    optimizer: str,
    scoremetric: str,
    version: str,
    accuracy: float,
    output_dir: PathLike = paths.FINAL_MODELS_DIR,
    tracker_file: PathLike = paths.MODEL_TRACKER,
) -> None:
    """
    Save a trained model with standardized naming conventions, and update a model tracker CSV.

    Args:
        model: Trained model object to be saved.
        model_type: Short name of the model (e.g., 'rf', 'xgb', 'mlp', 'ensemble').
        featureset: Feature subset used for model training (e.g., 'full', 'pca', 'lasso').
        optimizer: Optimization method applied (e.g., 'gridsearch', 'randomsearch', 'optuna').
        scoremetric: Primary metric optimized (e.g., 'acc', 'f1', 'rocauc').
        version: Version identifier for tracking iterations (e.g., 'v1', 'v2').
        accuracy: Accuracy (or other relevant score) achieved by the model.
        output_dir: Directory where the model file will be saved.
        tracker_file: Path for the model tracker CSV file.

    Raises:
        Exception: If saving or tracking fails.
    """
    try:
        output_dir_path = _resolve(output_dir)
        tracker_path = _resolve(tracker_file)

        paths.ensure_dir(output_dir_path)
        paths.ensure_dir(tracker_path.parent)

        filename = f"{model_type}_{featureset}_{optimizer}_{scoremetric}_{version}.joblib"
        save_path = output_dir_path / filename

        joblib.dump(model, save_path)

        tracker_columns = [
            "timestamp",
            "model_file",
            "model_type",
            "featureset",
            "optimizer",
            "scoremetric",
            "version",
            "accuracy",
        ]

        if tracker_path.exists():
            tracker_df = pd.read_csv(tracker_path)
        else:
            tracker_df = pd.DataFrame(columns=tracker_columns)

        new_entry = {
            "timestamp": pd.Timestamp.now().strftime("%Y-%m-%d %H:%M:%S"),
            "model_file": filename,
            "model_type": model_type,
            "featureset": featureset,
            "optimizer": optimizer,
            "scoremetric": scoremetric,
            "version": version,
            "accuracy": round(accuracy, 4),
        }

        if not tracker_df.empty:
            tracker_df = pd.concat(
                [tracker_df, pd.DataFrame([new_entry])], ignore_index=True
            )
        else:
            tracker_df = pd.DataFrame([new_entry])

        tracker_df.to_csv(tracker_path, index=False)

        print(f"Model successfully saved to {save_path}")
        print(f"Model tracker updated at {tracker_path}")

    except Exception as e:
        raise Exception(f"Error occurred while saving the model: {str(e)}")
